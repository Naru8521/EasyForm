 ...
 * @param {number} i
 */
export default async function EventAddForm(player, preset, showMode, i) {
    let showEvents = preset.showEvents;
    let event = showEvents[i];
    const form = new UI.ActionFormData();

    form.title("%ef.form.eventAdd");
    ...
    if (selection === 0) return await EventTypeForm(player, preset, showMode, i, EventAddForm);
    if (selection === 1) return await EventFilterForm(player, preset, showMode, i, EventAddForm);
    if (selection === 2) {
        showEvents[i] = event;
        await EventListForm(player, preset, showMode);
        return;

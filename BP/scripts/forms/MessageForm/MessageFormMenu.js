 ...
 */
export default async function MessageFormMenuForm(player, preset, showMode) {
    ...
    if (selection === 0) return await FormTitleForm(player, preset, showMode, MessageFormMenuForm);
    if (selection === 1) return await FormBodyForm(player, preset, showMode, MessageFormMenuForm);
